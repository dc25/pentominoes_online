...
function initTween() {
    ...
    position = { rotation: 0 };
    tween = new TWEEN.Tween(position).to({ rotation: 180 }, rotateTime).easing(TWEEN.Easing.Back.InOut).onComplete(function () {
        puzzleScene.loadNextSolution();
    });

    backtween = new TWEEN.Tween(position).to({ rotation: 0 }, rotateTime).easing(TWEEN.Easing.Back.InOut).onComplete(function () {
        puzzleScene.loadNextSolution();
    });

    pausetween0 = new TWEEN.Tween({}).to({}, pauseTime);

    pausetween1 = new TWEEN.Tween({}).to({}, pauseTime);
...
var PuzzleScene = (function () {
    ...
    PuzzleScene.prototype.updateColor = function (mesh, solution) {
        for (var s = 0; s < solution.length; s++) {
            var row = solution[s];
            var useColor = getColor(row[row.length - 1]);
            for (var i = 0; i < row.length - 1; i++) {
                var material = mesh[row[i]].material;
                material.color.set(useColor);
            }
        }
    };

    PuzzleScene.prototype.updateText = function (mesh, text) {
    ...
    PuzzleScene.prototype.initSolutionMeshes = function (solution, meshIndex) {
    ...
    PuzzleScene.prototype.addText = function (mesh, text) {
    ...
    PuzzleScene.prototype.initializeScene = function (data) {
    ...
    PuzzleScene.prototype.getPosition = function (id) {
        var x;
        var y;
        var z;
    ...
    PuzzleScene.prototype.update = function () {
        ...
        for (var meshindex = 0; meshindex < 2; meshindex++) {
    ...
    PuzzleScene.prototype.loadNextSolution = function () {
    ...
    PuzzleScene.prototype.renderScene = function () {
